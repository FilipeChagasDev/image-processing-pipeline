'''
TEST SCRIPT FOR CHANNEL HADAMARD
By Filipe Chagas
'''
...
my_pipeline.create_bus('mask', pl.BusFormat.Channel)
my_pipeline.create_bus('output', pl.BusFormat.Channel)

#pipes
my_had = hd.ChannelHadamardPipe()

#params
my_had.set_param('normalize_mask', False)
my_had.set_param('uniform_normalization', False)
my_had.set_param('clipping', False)
...
if __name__ == '__main__':
    for n in range(100):
        ...
        #expected output
        mask = np.array(np.random.rand(rd.randint(1,500),rd.randint(1,500))*255, np.uint8)
        mask_rs = cv.resize(mask, (in_data.shape[1], in_data.shape[0]))

        expected_out = in_data.astype(np.float) * mask_rs.astype(np.float)
